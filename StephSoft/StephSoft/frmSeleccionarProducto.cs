...
namespace StephSoft
{
    public partial class frmSeleccionarProducto : Form
    {
        ...
        private void BuscarRegistros(string Busqueda)
        {
            try
            {
                Producto DatosAux = new Producto { Opcion = TipoForm, Conexion = Comun.Conexion, NombreProducto = Busqueda, IDProveedor = string.IsNullOrEmpty(this._Datos.IDProveedor) ? string.Empty : this._Datos.IDProveedor, IDProductoXServicio = string.Empty };
        ...
        private void IniciarForm()
        {
            try
            {
                //if (TipoForm != 1 && TipoForm != 2 && TipoForm != 4 && TipoForm != 6)
                ...
                switch (TipoForm)
                {
                    ...
                    case 3: this.dgvProductos.Columns["NombreProducto"].HeaderText = "Producto / Servicio";
                        break;
                    case 4: this.dgvProductos.Columns["NombreProducto"].HeaderText = "Producto";
                        break;
                    case 6: this.dgvProductos.Columns["NombreProducto"].HeaderText = "Producto / Herramienta";
                        break;
                    default:
                        break;
                }
        ...
        private Producto ObtenerDatos(int Fila)
        {
            try
            {
                ...
                if (Fila < this.dgvProductos.Rows.Count)
                {
                    ...
                    if (TipoForm == 12)
                    {
                        Aux.MetricaDesc = string.IsNullOrEmpty(this.dgvProductos.Rows[Fila].Cells["Metrica"].Value.ToString()) ? string.Empty : this.dgvProductos.Rows[Fila].Cells["Metrica"].Value.ToString();
                    }
                    if (TipoForm == 11)
                    {
                        int IDTipoProducto = 0;
                        int.TryParse(this.dgvProductos.Rows[Fila].Cells["IDTipoProducto"].Value.ToString(), out IDTipoProducto);
                        Aux.IDTipoProducto = IDTipoProducto;
                    }
                }
                return Aux;
            }
