...
namespace StephSoft
{
    public partial class frmCatClientes : Form
    {
        ...
        private void btnCanjearTR_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.dgvClientes.SelectedRows.Count == 1)
                {
                    ...
                    if (!string.IsNullOrEmpty(DatosAux.IDCliente))
                    {
                        frmCanjearTarjetaRegalo ActivarTarjeta = new frmCanjearTarjetaRegalo(DatosAux);
                        ActivarTarjeta.ShowDialog();
                        ActivarTarjeta.Dispose();
                        if (ActivarTarjeta.DialogResult == DialogResult.OK)
                        {
                            int Row = this.dgvClientes.Rows.GetFirstRow(DataGridViewElementStates.Selected);
                            this.dgvClientes.Rows[Row].Cells["Saldo"].Value = ActivarTarjeta.DatosAux.Saldo;
                        }
                    }
                }
                else
