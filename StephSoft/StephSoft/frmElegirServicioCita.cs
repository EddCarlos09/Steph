        ...
        private List<Error> ValidarDatos()
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(this._ServicioElegido.IDServicio))
                    ListaErrores.Add(new Error { Numero = (Aux += 1), Descripcion = "Seleccione un Servicio.", ControlSender = this.cmbServicio });
                if(string.IsNullOrEmpty(this._ClienteSeleccionado.IDCliente))
                    ListaErrores.Add(new Error { Numero = (Aux += 1), Descripcion = "Seleccione un Cliente.", ControlSender = this.cmbCliente });
                return ListaErrores;
            }
            catch (Exception ex)
            {
