...
namespace StephSoft
{
    public partial class frmElegirProveedor : Form
    {
        ...
        private List<Proveedor> _DatosActuales;
        public List<Proveedor> DatosActuales
        {
        ...
        public frmElegirProveedor()
        {
            ...
            {
                LogError.AddExcFileTxt(ex, "frmElegirProveedor ~ frmElegirProveedor()");
        ...
        private void Busqueda(string TextoBusqueda)
        {
            try
            {
                ...
                {
                    List<Proveedor> Resultados = this.DatosActuales.FindAll(x => x.RazonSocial.ToUpper().Contains(TextoBusqueda.ToUpper()));
                    this.dgvProveedor.DataSource = Resultados;
                }
                else
                {
                    this.dgvProveedor.DataSource = this.DatosActuales;
                }
            }
            catch (Exception ex)
            {
        ...
        private Proveedor ObtenerDatos(int Index)
        {
            try
            {
                DataGridViewRow Fila = this.dgvProveedor.Rows[Index];
                Proveedor Datos = new Proveedor();
                Datos.IDProveedor = Fila.Cells["IDProveedor"].Value.ToString();
                Datos.RazonSocial = Fila.Cells["RazonSocial"].Value.ToString();
                return Datos;
            }
            catch (Exception ex)
            {
        ...
        private void btnCancelarBusqueda_Click(object sender, EventArgs e)
        {
            try
            {
                this.dgvProveedor.DataSource = DatosActuales;
                this.txtBusqueda.Text = string.Empty;
                this.txtBusqueda.Focus();
            }
            catch (Exception ex)
        ...
        private void dgvTarjetas_KeyUp(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    if (this.dgvProveedor.SelectedRows.Count == 1)
                    {
                        Int32 RowSelected = this.dgvProveedor.Rows.GetFirstRow(DataGridViewElementStates.Selected);
